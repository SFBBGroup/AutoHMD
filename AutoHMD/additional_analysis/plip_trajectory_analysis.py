...
def extract_residue_number(res_str):
    """Extract numeric residue number from string like 'Ser32'."""
...
def plot_interaction_map(prevalence_df, output_file, min_prevalence=10):
    ...
    ag_res = sorted(df_f['lig_res'].unique(), key=extract_residue_number)
    ab_pos = {r: i for i, r in enumerate(ab_res)}
    ag_pos = {r: i for i, r in enumerate(ag_res)}

    # Map each antibody residue to its chain
    ab_res_to_chain = {}
    for _, row in df_f.iterrows():
        ab_res_to_chain[row['prot_res']] = row['prot_chain']
    ...
    for res, pos in ab_pos.items():
        chain = ab_res_to_chain.get(res, 'H')
        color = chain_colors.get(chain, 'skyblue')
        ax1.scatter(0, pos, s=145, c=color, edgecolor='black', zorder=2)
        ax1.text(-0.05, pos, res, ha='right', va='center', fontsize=16, fontweight='bold')

    for res, pos in ag_pos.items():
        ax1.scatter(1, pos, s=150, c='gray', edgecolor='black', linewidth=1.5, zorder=2)
        ax1.text(1.05, pos, res, ha='left', va='center', fontsize=16, fontweight='bold')

    ax1.set_xlim(-0.3, 1.3)
    ax1.set_ylim(-1, max_y)
    ax1.axis('off')
    ax1.text(0, max_y + 0.5, 'Antibody', ha='center', fontsize=18, fontweight='bold')
    ax1.text(1, max_y + 0.5, 'Antigen',  ha='center', fontsize=18, fontweight='bold')

    # Combined legend: chain colors + interaction types
    chain_legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w',
                   markerfacecolor=chain_colors.get(ch, 'gray'), markersize=12,
                   markeredgecolor='black', markeredgewidth=1.5,
                   label=f'Chain {ch}', linestyle='None')
        for ch in sorted(set(ab_res_to_chain.values()))
    ]
    int_legend_elements = [
        plt.Line2D([0], [0], color=c, linewidth=3, label=t)
        for t, c in int_type_colors.items() if t in df_f['type'].values
    ]
    ax1.legend(
        handles=chain_legend_elements + int_legend_elements,
        loc='upper center',
        bbox_to_anchor=(0.5, -0.02),
        ncol=min(3, len(chain_legend_elements) + len(int_legend_elements)),
        fontsize=16,
        framealpha=0.95,
        edgecolor='black',
        title='Antibody Chains & Interaction Types',
        title_fontsize=16,
        columnspacing=1.5,
        handletextpad=0.5,
    )

    # Right panel: table with Ab Chain column
    ...
    t = ax2.table(
        cellText=table_data,
        colLabels=['#', 'Ab. Res', 'Ab. Chain', 'Ag. Res', 'Type', 'Prev.'],
        cellLoc='center',
        loc='center',
        colWidths=[0.08, 0.15, 0.15, 0.15, 0.25, 0.15],
    )
